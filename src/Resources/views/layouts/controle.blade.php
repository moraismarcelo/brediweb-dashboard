...
<body>
    ...
    $(document).ready(function(){
        ...
        if ($('.tinymce').length > 0) {
            $.each($('.tinymce'), function(index, el){
                startEditor(index, el);
            });

        }

        function startEditor(index, el) {
            ...
            tinymce.init({
                ...
                // images_upload_url: '{{ route('bredidashboard::controle.summernote.upload') }}?_token={{ csrf_token() }}',
                images_upload_handler: function (blobInfo, success, failure) {
                    var xhr, formData;
                    xhr = new XMLHttpRequest();
                    xhr.withCredentials = false;
                    xhr.open('POST', '{{ route('bredidashboard::controle.summernote.upload') }}?_token={{ csrf_token() }}');
